...
def calibracion():
    ...
    array_chapters = ['ARQUITECTURA DE NEGOCIOS']

    for chapter in array_chapters:
        ...
        query_resultados = '''
            ...
            ;WITH BASE_COLABORADOR AS(
                SELECT DISTINCT FRE.MATRICULA,UC.Nombres + ' ' + UC.Ape_Paterno + ' ' + UC.Ape_Materno NOMBRE_COMPLETO
                FROM BCP_GDH_PA_DW.PDIGITAL.F_RESULTADO_EXPERTISE FRE
                LEFT JOIN BCP_GDH_DW_UGI.UGI.COLABORADORES UC ON FRE.MATRICULA=RIGHT(UC.MATRICULA,6)
                WHERE FK_FECHA = @FECHA_EVA AND FK_CHAPTER=@PK_CHAPTER
            ),
            BASE_LIDER AS(
                SELECT DISTINCT FRE.MATRICULA_CALIFICADOR,UC.Nombres + ' ' + UC.Ape_Paterno + ' ' + UC.Ape_Materno NOMBRE_COMPLETO
                FROM BCP_GDH_PA_DW.PDIGITAL.F_RESULTADO_EXPERTISE FRE
                LEFT JOIN BCP_GDH_DW_UGI.UGI.COLABORADORES UC ON FRE.MATRICULA_CALIFICADOR=RIGHT(UC.MATRICULA,6)
        ...
        if duplicados.empty:
            ...
            if not(diferencias_df.empty):
                ...
                for fila in range(len(diferencias_df_new.index)):
                    ...
                    if cont >= 4: diferencias_df_new.at[fila:fila, 'Concatenadas_2'] = 'Subió 4 o más capacidades'
